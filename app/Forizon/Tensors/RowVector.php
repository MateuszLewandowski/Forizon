...
class RowVector extends Vector implements Vectorable, Tensor
{
    /**
     * @param  array[]  $data
    ...
    public function __construct(array $data = [], bool $skip = false)
    {
        try {
            if (empty($data)) {
                throw new InvalidArgumentException('Row vector cannot be empty.', Response::HTTP_BAD_REQUEST);
            }
            $data = array_values($data);
            $this->columns = count($data);
            $this->length = $this->columns;
            $this->rows = 1;
            if (! $skip) {
    ...
    {
        return new ColumnVector($this->data);
    }
    ...
    {
        TensorValidator::areIdentical($this->columns, $matrix->columns);
     ...
     */
    public function isEqual(mixed $tensor): self
    {
        return $this->run(__FUNCTION__, $tensor);
    }

    public function isNotEqual(mixed $tensor): self
    {
        return $this->run(__FUNCTION__, $tensor);
    }
     ...
     */
    public function sin(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }

    public function asin(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }

    public function cos(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }

    public function acos(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }

    public function tan(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }

    public function atan(): mixed
    {
        return self::fastCreate($this->map(__FUNCTION__));
    }
    ...
    {
        try {
            return new self(parent::randmax($length), true);
        } catch (InvalidArgumentException $e) {
            Log::critical($e->getMessage(), [__CLASS__]);
            throw $e;
        }
    ...
    {
        try {
            return new self(parent::random($length), true);
        } catch (InvalidArgumentException $e) {
            Log::critical($e->getMessage(), [__CLASS__]);
            throw $e;
        }
    ...
    {
        try {
            return new self(parent::uniform($length), true);
        } catch (InvalidArgumentException $e) {
            Log::critical($e->getMessage(), [__CLASS__]);
            throw $e;
        }
    ...
    {
        try {
            return new self(parent::gaussian($length), true);
        } catch (InvalidArgumentException $e) {
            Log::critical($e->getMessage(), [__CLASS__]);
            throw $e;
        }
    ...
    final public static function fillRandomizeRequireFrom(int $length, int $from = -1, int $to = 1, int $precision = 2): self
    {
        try {
    ...
    final public static function fillRandomizeRequireTo(int $length, int $from = -1, int $to = 1, int $precision = 2): self
    {
        try {
